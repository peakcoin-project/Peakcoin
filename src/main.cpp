...
#if defined(NDEBUG)
# error "Peakcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xaf11de684a0d190f192602f83b301b2ff4723f2b164006704019ed79dbcdcb9c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Peakcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Peakcoin Signed Message:\n";
...
{
    // Peakcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 12.6 * COIN;
	
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 60; // Peakcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 60; // Peakcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Peakcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Peakcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xaf11de684a0d190f192602f83b301b2ff4723f2b164006704019ed79dbcdcb9c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "01/Mar/2015 Boris Nemtsov murder: Protest march turns critical of Putin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046684710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1425200400;
        ...
        assert(block.hashMerkleRoot == uint256("0x711571e89b5c87ea3fd2b702756f4745d6d87dc1340b4cd19fa086054af183f4"));
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // Peakcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PeakcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PeakcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PeakcoinMiner(CWallet *pwallet)
{
    printf("PeakcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("peakcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PeakcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PeakcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PeakcoinMiner, pwallet));
}
